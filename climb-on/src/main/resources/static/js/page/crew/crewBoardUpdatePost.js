    ...
    if (resultTxt.length === 0) {
        ...
        if(remainingFiles.length > 0){
            try {
                ...
                //need change to append
                // hiddenImgUrlInput.value = [...new Set((hiddenImgUrl + "," + data).split(','))].join(',');
                // console.log(hiddenImgUrlInput);
                // console.log(hiddenImgUrlInput.value);
        ...
        const updatedContentHTML = quill.root.innerHTML;
        const updatedDoc = parser.parseFromString(updatedContentHTML, 'text/html');
        const updatedImgTags = Array.from(updatedDoc.getElementsByTagName('img'));
        const imgs = [];
        updatedImgTags.forEach((img)=>{
            imgs.push(img.src);
        });
        hiddenImgUrlInput.value = imgs;
